...
;( function( window ) {
	...
	FForm.prototype._addControls = function() {
		...
		this.ctrlPrevius = createElement( 'button', { cName : 'fs-anterior', inner : 'Anterior', appendTo : this.ctrls } );
	...
	FForm.prototype._initEvents = function() {
		...
		this.ctrlPrevius.addEventListener( 'click', function() {
			self._prevField(); 
		} );
				...
				switch( input.tagName.toLowerCase() ) {
					...
					case 'input' : 
						[].slice.call( fld.querySelectorAll( 'input[type="radio"]' ) ).forEach( function( inp ) {
							...
							inp.addEventListener( 'mouseover', function(ev) { ev.preventDefault(); inputOver(this.id); } );
							inp.addEventListener( 'mouseout', function(ev) { ev.preventDefault();inputOut(this.id);  } );
							inp.addEventListener( 'click', function(ev) { 	ev.preventDefault(); inputClick(this.id) ; self._nextField(); } );
	...
	FForm.prototype._nextField = function( backto ) {
		...
		this._showCtrl( this.ctrlPrevius );
		...
		var self = this,
			onEndAnimationFn = function( ev ) {
				...
				if( self.isLastStep ) {
					...
					//self._hideCtrl( self.ctrlPrevius );
	...
	FForm.prototype._validade = function() {
		...
		switch( input.tagName.toLowerCase() ) {
			...
			case 'textarea' :
				if( input.value === '' ) {
					error = 'NOVAL';
				}else{ //no error pero tinc que guarda el valor
					textEditSave(input);
				}
				break;
		}
	 ...
	 * jumps to the next field
	 */
	FForm.prototype._prevField = function( backto ) {
		if( this.isLastStep  || this.isAnimating ) {
			...
			if (this.current==0){
				 //this._hideCtrl( this.ctrlPrevius );
			}
